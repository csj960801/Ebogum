...
<div class="custom_container Admin_wrapper">
	...
	<div class="main_col col-md-10 col-xs-10">
		<div class="main_board_wrapper">
			...
			<input type="hidden" id="adminPageCnt" value="1" />
			 ...
			 <form id="adminFrm" class="adminFrm" name="adminFrm">
				<table class="table admin_table">
					...
					<tbody class="Admintbody">
						<tr>
							<td colspan="9">
								<button id="memberManageBtn" class="adminBtn btn btn-sm">회원관리</button>
								<button id="memberDelBtn" class="adminBtn btn btn-sm">회원삭제</button>
								<button id="paymentBtn" class="adminBtn btn btn-sm">결제현황</button>
								<button id="paymentRefreshBtn" class="adminBtn btn btn-sm">결제현황
									새로고침</button>
							</td>
						</tr>
						<tr>
							<td colspan="9"><h4 class="adminManage_table_title">회원 관리</h4></td>
						</tr>
						<tr class="search_tr">
							<td><input type="text" placeholder="검색하실 ID를 입력해주세요." name="adminSearchId" class="adminform" id="adminSearchId" /><button id="manager_SearchbyId_btn" class="adminBtn btn btn-sm">아이디로 검색</button></td>
						    <td><input type="text" placeholder="검색하실 이름을 입력해주세요." name="adminSearchName" class="adminform" id="adminSearchName" /><button id="manager_SearchbyName_btn" class="adminBtn btn btn-sm">이름으로 검색</button></td>
												...
												<tbody class="tbody">
													<c:if test="${memberlist != null}">
									    			  <c:forEach items="${memberlist}" var="managingMember" varStatus="status">
									    				 <tr>
									    				   <td>${status.count}</td>
									    				   <td>${managingMember.duplicateid}</td>
									    				   <td>${managingMember.pass1}</td>
									    				   <td>${managingMember.name}</td>
									    				   <td>${managingMember.date}</td>
									    				   <td></td>
									    				   <td></td>
									    				   <td></td>
									    				   <td></td>
