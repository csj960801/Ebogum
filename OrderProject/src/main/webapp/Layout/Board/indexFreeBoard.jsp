	...
	<dd class="freeboard_dd">
		<div class="freeboard_wrapper">
			<%
				while (rs.next()) {
			%>
			<a href="#" onclick="javascript:freeboard_Read('<%=rs.getString("FreeboardCnt")%>', '<%=rs.getString("writer")%>' , '<%=rs.getString("title")%>','<%=rs.getString("content")%>','<%=rs.getString("hit")%>','<%=rs.getString("boardParam")%>','<%=rs.getString("boarddate")%>')"
